...
from windy.classes.training import TrainingEnv as env
from windy.classes.valuemodel import ValueModel, reload_model
from windy.classes.heuristicpolicy import HeuristicPolicy
from windy.classes.policy import policy_name

# import default globals
from windy.learn.parameters.__defaults import *
...
def build_new_model(
    ...
    model.build_graph(input_shape_nobatch=MY_TRAINING_ENV.NN_input_shape)
...
def save_parameters(env, model):
    ...
    for out in (None, pfile):
        ...
        print("R_BAR = " + str(env.R_bar), file=out)
        print("V_BAR = " + str(env.V_bar), file=out)
        print("TAU_BAR = " + str(env.tau_bar), file=out)
        print("LAMBDA_BAR = " + str(env.lambda_bar), file=out)
        print("NORM_POISSON = " + env.norm_Poisson, file=out)
        print("GRID = " + str(env.shape), file=out)
        ...
        print("* Tricks to accelerate learning", file=out)
        print("DISCOUNT = " + str(DISCOUNT), file=out)
        print("REWARD_SHAPING = " + str(REWARD_SHAPING), file=out)
...
def _Worker(episode, policy, eps, memorize, eval):
    ...
    2. The agent moves according to the model
    3. The episode terminates when the agent has almost certainly found the source
    ...
    while True:
        ...
        if SYM_TRAIN_RANDOMIZE and memorize:
            if myenv.Ndim != 2:
                raise Exception("SYM_TRAIN_RANDOMIZE only implemented in 2D")
            if myenv.Ndim == 2:
                Nsym = 2
...
def train_model(eps_floor, eps_0, eps_decay, max_it, ref_stats):
    ...
    # before training starts, populate memory with data from initial policy
    Nepisodes = max(int(0.1 * MEMORY_SIZE / STOP_t), 1)
...
if __name__ == '__main__':
    ...
    if CONTINUE_TRAINING:
        ...
        oldmodel = reload_model(MODEL_PATH, inputshape=MY_TRAINING_ENV.NN_input_shape)
